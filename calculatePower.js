// Write a function to calculate the power of a number.
// Instructions:
// The power of a number is calculated by multiplying the base with itself for exponent times.
// For example, a base of 2 raised to the exponent of 4 is 2^4 = 2 * 2 * 2 * 2 = 16.
// Return the power of base raised to exponent.
// Example:
// For this input base = 2, exponent = 3, the result should be: 8
// Reason: The base 2 raised to the exponent 3 results in 8.

function calculatePower(base, exponent) {
  return Math.pow(base, exponent);
}
